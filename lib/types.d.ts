...
/** Represents the default {@link BotCommandScope|scope} of bot commands. Default commands are used if no commands with a narrower scope are specified for the user. */
...
interface MenuButtonWebApp { 
	...
	/** Text on the button */
	text: string,
	/** Description of the Web App that will be launched when the user presses the button. The Web App will be able to send an arbitrary message on behalf of the user using the method {@link T.answerWebAppQuery|answerWebAppQuery}. */
	web_app: WebAppInfo
}
...
interface InlineQueryResultGif { 
	...
	caption?: string,
	/** Mode for parsing entities in the caption. See  for more details. */
	parse_mode?: string,
	/** List of special entities that appear in the caption, which can be specified instead of parse_mode */
	caption_entities?: MessageEntity,
	/**  attached to the message */
...
interface ChosenInlineResult { 
	...
	location?: Location,
	/** Identifier of the sent inline message. Available only if there is an {@link InlineKeyboardMarkup|inline} attached to the message. Will be also received in {@link CallbackQuery|callback} and can be used to  the message. */
	inline_message_id?: string,
...
interface SentWebAppMessage { 
	/** Identifier of the sent inline message. Available only if there is an {@link InlineKeyboardMarkup|inline} attached to the message. */
	inline_message_id?: string
}
...
interface LabeledPrice { 
	/** Portion label */
	label: string,
	/** Price of the product in the smallest units of the  (integer, not float/double). For example, for a price of US$ 1.45 pass amount = 145. See the exp parameter in , it shows the number of digits past the decimal point for each currency (2 for the majority of currencies). */
	amount: number
}
...
interface Invoice { 
	...
	currency: string,
	/** Total price in the smallest units of the currency (integer, not float/double). For example, for a price of US$ 1.45 pass amount = 145. See the exp parameter in , it shows the number of digits past the decimal point for each currency (2 for the majority of currencies). */
	total_amount: number
}
...
interface EncryptedPassportElement { 
	...
	type: string,
	/** Base64-encoded encrypted Telegram Passport element data provided by the user, available for “personal_details”, “passport”, “driver_license”, “identity_card”, “internal_passport” and “address” types. Can be decrypted and verified using the accompanying {@link EncryptedCredentials|EncryptedCredentials}. */
	data?: string,
	/** User's verified phone number, available only for “phone_number” type */
	phone_number?: string,
	/** User's verified email address, available only for “email” type */
	email?: string,
	/** Array of encrypted files with documents provided by the user, available for “utility_bill”, “bank_statement”, “rental_agreement”, “passport_registration” and “temporary_registration” types. Files can be decrypted and verified using the accompanying {@link EncryptedCredentials|EncryptedCredentials}. */
	files?: PassportFile,
	/** Encrypted file with the front side of the document, provided by the user. Available for “passport”, “driver_license”, “identity_card” and “internal_passport”. The file can be decrypted and verified using the accompanying {@link EncryptedCredentials|EncryptedCredentials}. */
	front_side?: PassportFile,
	/** Encrypted file with the reverse side of the document, provided by the user. Available for “driver_license” and “identity_card”. The file can be decrypted and verified using the accompanying {@link EncryptedCredentials|EncryptedCredentials}. */
	reverse_side?: PassportFile,
	/** Encrypted file with the selfie of the user holding a document, provided by the user; available for “passport”, “driver_license”, “identity_card” and “internal_passport”. The file can be decrypted and verified using the accompanying {@link EncryptedCredentials|EncryptedCredentials}. */
	selfie?: PassportFile,
...
/** Describes data required for decrypting and authenticating {@link EncryptedPassportElement|EncryptedPassportElement}. See the Telegram Passport Documentation for a complete description of the data decryption and authentication processes. */
...
interface Game { 
	...
	photo: PhotoSize,
	/** Brief description of the game or high scores included in the game message. Can be automatically edited to include current high scores for the game when the bot calls {@link T.setGameScore|setGameScore}, or manually edited using {@link T.editMessageText|editMessageText}. 0-4096 characters. */
	text?: string,
